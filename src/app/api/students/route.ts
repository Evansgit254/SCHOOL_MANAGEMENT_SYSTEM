...
export async function POST(req: NextRequest) {
  try {
    ...
    const rl = await rateLimitConsumeAsync(`students:POST:${userId || ip}`, { tokensPerInterval: 10, intervalMs: 60_000 });
    ...
    captureError(error, { route: 'POST /api/students' });
...
export async function PUT(req: NextRequest) {
  try {
    const { userId } = await auth();
    const ip = getClientIp(req);
    const rl = await rateLimitConsumeAsync(`students:PUT:${userId || ip}`, { tokensPerInterval: 10, intervalMs: 60_000 });
    if (!rl.allowed) return NextResponse.json({ error: 'Too Many Requests' }, { status: 429 });
    const data = studentSchema.parse(await req.json());
    const result = await updateStudent({ success: false, error: false }, data);
    return NextResponse.json(result);
  } catch (error) {
    captureError(error, { route: 'PUT /api/students' });
    return NextResponse.json({ error: 'Server error', details: (error as Error)?.message || error }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const result = await deleteStudent({ success: false, error: false }, payload!);
    return NextResponse.json(result);
  } catch (error) {
    captureError(error, { route: 'DELETE /api/students' });
    return NextResponse.json({ error: 'Server error', details: (error as Error)?.message || error }, { status: 500 });
  }
} 